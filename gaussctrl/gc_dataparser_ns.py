...
class GaussCtrlDataParser(DataParser):
    ...
    def _generate_parser_uco(self, uco_data, split):
        ...
        for i in tqdm(range(len(uco_data['data'])), desc='Loading uco3d data'):
        #for i in tqdm(range(200),desc='Loading uco3d data'):
            
            frame=next(iterator)
            image_filenames.append(str(int(frame['frame_number'])))
            #mask_filenames.append(str(int(frame.frame_number)))
            #depth_filenames.append(str(int(frame.frame_number)))
        
            #fname = self._get_fname(filepath, data_dir)
            camera=frame['camera']
            image_size_tensor = torch.tensor([512, 512])[None].repeat(1, 1)

            Rcv, tvec, camera_matrix = opencv_cameras_projection_from_uco3d(camera, image_size=image_size_tensor)
            pose = torch.cat((torch.cat((Rcv[0], tvec[0].unsqueeze(1)), dim=1), torch.tensor([[0.0, 0.0, 0.0, 1.0]])), dim=0)
            
            if not fx_fixed:
                fx.append(float(camera_matrix[0,0,0]))
            if not fy_fixed:
                fy.append(float(camera_matrix[0,1,1]))
            if not cx_fixed:
